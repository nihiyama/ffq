...
func BenchmarkGoJSONSimpleQueueEnqueueDequeue(b *testing.B) {
		...
		b.Run(fmt.Sprintf("Size%d", tt), func(b *testing.B) {
			...
			for i := 0; i < b.N; i++ {
				...
				q, err := ffq.NewQueue[BenchmarkData](
					...
					ffq.WithMaxPage(3),
					ffq.WithEncoder(json.Marshal),
					ffq.WithDecoder(json.Unmarshal),
				)
				if err != nil {
					panic(err)
				}
...
func BenchmarkGoJSONSimpleQueueBulkEnqueueDequeue(b *testing.B) {
	var size uint64 = 100
...
func BenchmarkGoJSONSimpleQueueEnqueueDequeue_5MP(b *testing.B) {
	for _, tt := range tests {
		b.Run(fmt.Sprintf("Size%d", tt), func(b *testing.B) {
			dir, _ := os.MkdirTemp("", "ffqbenchtest")
			data := createData(tt)
...
func BenchmarkGoJSONSimpleQueueBulkEnqueueDequeue_5MP(b *testing.B) {
	var size uint64 = 100
	lazy := 10 * time.Millisecond
	for _, tt := range tests {
		b.Run(fmt.Sprintf("Size%d", tt), func(b *testing.B) {
...
func BenchmarkGoJSONGroupQueueEnqueueDequeue_5Group(b *testing.B) {
...
func BenchmarkGoJSONGroupQueueBulkEnqueueDequeue_5Group(b *testing.B) {
	testQueues := []string{"queue1", "queue2", "queue3", "queue4", "queue5"}
	size := 100
	lazy := 10 * time.Millisecond
	for _, tt := range tests {
